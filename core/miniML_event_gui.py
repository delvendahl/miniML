...
class minimlGuiMain(QMainWindow):
    ...
    def initUI(self):
        ...
        # Test what happens when I re-write this to matplotlib
        ...
        self.tracePlot = MatplotlibWidget()
        self.ax = self.tracePlot.getFigure().add_subplot(111)
        ...
        self.splitter3.setSizes([750, 500])
    ...
    def _create_toolbar(self):
        ...
        self.analyseAction = QAction(QIcon("icons/rocket_launch_24px_blue.svg"), "Analyse", self) # rocket_launch_24px_blue
    ...
    def _connect_actions(self):
        ...
        self.event_paramAction.triggered.connect(self.toggle_event_param_win)
    ...
    def _init_splitter_variables(self) -> None:
        '''
        Init the "_store_size_" variables.
        splitter_1: overlay panels subplots; eventPlot, AveragePlot, HistogramPlot
        splitter_2: prediction, data and event overlay panels
        splitter_3: table panel
        '''
        # self._store_size = self.splitter2.sizes()
        self._store_size_a = self.splitter2.sizes()[0] # prediction panel
        self._store_size_b = self.splitter2.sizes()[1] # ephys data panel
        self._store_size_c = self.splitter2.sizes()[2] # overlay panels
        self._store_size_c1 = self.splitter1.sizes()[0] # eventPlot
        self._store_size_c2 = self.splitter1.sizes()[1] # AveragePlot
        self._store_size_c3 = self.splitter1.sizes()[2] # HistogramPlot
        self._store_size_d = self.splitter3.sizes() # Table panel

        print(self.splitter2.sizes())
        print(self._store_size_a)
        print(self._store_size_b)
        print(self._store_size_c)


    def toggle_table_win(self) -> None:
        '''
        Toggle table panel on/off
        '''
        if 0 in self.splitter3.sizes():
            self.splitter3.setSizes(self._store_size_d)
        else:
            self._store_size_d = self.splitter3.sizes() # _store_size_d: 'd' refers to table win (used to be 'c')
            self.splitter3.setSizes([np.sum(self.splitter3.sizes()), 0])

    def toggle_plot_win(self) -> None:
        '''
        Toggle overlay/average/histogram on/off
        '''
        sizes = self.splitter2.sizes()
        if sizes[2] == 0: # panel is hidden
            sizes[0] = 0 if sizes[0] == 0 else self._store_size[0]
            sizes[1] = (np.sum(sizes[0:3]) - self._store_size_c) if sizes[0] == 0 else (self._store_size[1] - self._store_size_c)
            sizes[2] = self._store_size_c # _store_size_c: 'c' refers to event overlay window, used to be c
            self._store_size = sizes
        else: # panel is shown
            self._store_size = sizes
            self._store_size_c = sizes[2]
    ...
    def toggle_prediction_win(self) -> None:
        ...
        if sizes[0] == 0: # panel is hidden
            sizes[0] = self._store_size_a # 'a' refers to prediction win.
    ...
    def toggle_event_param_win(self) -> None:
        if self._event_viewer_on:
            # Restore original window
            self._event_viewer_on = False
            self.splitter1.setSizes(self._splitter1_sizes)
            self.splitter2.setSizes(self._splitter2_sizes)
            self.update_main_plot()
            ...
            # Set event viewer.
            self._event_viewer_on = True
            self._splitter1_sizes = self.splitter1.sizes()
            self._splitter2_sizes = self.splitter2.sizes()
            self.splitter2.setSizes([0, np.sum(self._splitter2_sizes), 0])
            if self.was_analyzed:
                self._init_event_viewer()
            
    def _init_event_viewer(self):
        # These I will have to set differently.
        self.ind = 0
        self.lw=0.8
        ...
        self.ax.plot(time_ax, data, c='k', alpha=0.5, lw=self.lw)
        self.ax.plot(time_ax, filtered_data, c='k', lw=self.lw)

        self.ax.scatter(rel_bsl_start, bsl, c='r', zorder=2)
        self.ax.scatter(rel_bsl_end, bsl, c='r', zorder=2)
        self.ax.hlines(bsl,
                rel_bsl_start,
                rel_peak_loc, color='r', zorder=2, ls='--', lw=self.lw*2,
                label='baseline')

        self.ax.scatter(rel_min_rise, self.filtered_data[min_rise], c='magenta', zorder=2)
        self.ax.scatter(rel_max_rise, self.filtered_data[max_rise], c='magenta', zorder=2)
        self.ax.hlines(self.filtered_data[min_rise],
                rel_min_rise,
                rel_max_rise, color='magenta', zorder=2, ls='--', lw=self.lw*2)
        self.ax.vlines(rel_max_rise,
                self.filtered_data[min_rise],
                self.filtered_data[max_rise], color='magenta', zorder=2, ls='--', lw=self.lw*2,
                label='10-90% risetime')


        # self.ax.scatter(rel_peak_loc, self.filtered_data[peak_loc], c='orange', zorder=2)    
        self.ax.scatter(rel_peak_loc_left, peak_val, c='orange', zorder=2, marker = 'x')   
        self.ax.scatter(rel_peak_loc_right, peak_val, c='orange', zorder=2, marker = 'x')    
        self.ax.scatter(rel_peak_loc, peak_val, c='orange', zorder=2)    
        
        
        self.ax.vlines(rel_peak_loc,
                peak_val,
                peak_val - self.detection.event_stats.amplitudes[self.ind],
                color='orange', zorder=2, ls='--', lw=self.lw*2,
                label='amplitude (peak-baseline)')
        
        if len(peaks_in_win):
            self.ax.scatter(rel_peaks_in_win, self.filtered_data[peaks_in_win], c='orange', zorder=2)    


        if not np.isnan(self.detection.half_decay[self.ind]):
            self.ax.scatter(rel_decay_loc, self.filtered_data[decay_loc], c='green', zorder=2)
            self.ax.hlines(self.filtered_data[decay_loc],
                    rel_peak_loc,
                    rel_decay_loc, color='green', zorder=2, ls='--', lw=self.lw*2,
                    label='half decay time')
        ...
        self.ax.set_ylabel(f'{self.detection.trace.y_unit}')
        self.ax.set_title(f'event # {self.ind}/{self.num_events}')
