...
class TestEventDetection(unittest.TestCase):
    ...
    def test_hann_filter(self):
        ...
        self.assertTrue(
            np.array_equal(self.detector.hann_filter(self.data, 0), self.data)
        )
    ...
    def test_full_detection_flow(self):
        ...
        # Create mock data with length 5000, and a synthetic event at index 2000
        mock_data = np.zeros(5000, dtype=float)
        mock_data[2000:2300] = np.linspace(-20, 0, 300)
        ...
        with patch.object(EventDetection, "_EventDetection__predict", mock_predict_fn):
            self.detector.trace.data = mock_data
            # Run detection
        ...
        self.assertEqual(
            len(self.detector.event_stats.amplitudes),
            len(self.detector.event_locations),
        )
    ...
    def test_serialization_methods(self):
        """Test saving detection results to H5, and CSV files."""
        ...
        try:
            ...
            with h5py.File(h5_path, "r") as f_h5:
                ...
                self.assertAlmostEqual(
                    f_h5.attrs["recording_time"], self.trace.total_time
                )
        finally:
