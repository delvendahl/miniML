...
class EventDetection:
    ...
    def __init__(
        ...
        self._event_locations = np.array([])
        self._event_scores = np.array([])
        self._events = np.array([])
        self._event_peak_locations = np.array([])
        self._bsl_starts = np.array([])
        self._bsl_ends = np.array([])
        self._event_start = np.array([])
        self._event_peak_values = np.array([])
        self._event_bsls = np.array([])
        self._event_bsl_durations = np.array([])
        self._decaytimes = np.array([])
        self._charges = np.array([])
        self._risetimes = np.array([])
        self._half_decay = np.array([])
        self._halfwidths = np.array([])
        self._rise_half_amp_times = np.array([])
        self._decay_half_amp_times = np.array([])
        self._min_positions_rise = np.array([])
        self._max_positions_rise = np.array([])
        self._min_values_rise = np.array([])
        self._max_values_rise = np.array([])
    ...
    def detect_events(
        ...
        if self.event_locations.shape[0] > 0:
            ...
            for ix in range(self.event_locations.shape[0]):
                ev = Event(
                    location=self.event_locations[ix],
                    score=self.event_scores[ix],
                    waveform=self.events[ix],
                    peak_location=self.event_peak_locations[ix],
                    peak_value=self.event_peak_values[ix],
                    bsl_start=self.bsl_starts[ix],
                    bsl_end=self.bsl_ends[ix],
                    bsl_value=self.event_bsls[ix],
                    bsl_duration=self.event_bsl_durations[ix],
                    onset_location=self.event_start[ix],
                    decaytime=self.decaytimes[ix],
                    charge=self.charges[ix],
                    risetime=self.risetimes[ix],
                    half_decay=self.half_decay[ix],
                    halfwidth=self.halfwidths[ix],
                    rise_half_amp_time=self.rise_half_amp_times[ix],
                    decay_half_amp_time=self.decay_half_amp_times[ix],
                    min_position_rise=self.min_positions_rise[ix],
                    max_position_rise=self.max_positions_rise[ix],
                    min_value_rise=self.min_values_rise[ix],
                    max_value_rise=self.max_values_rise[ix],
                    slope=self.slopes[ix],
                    excluded=False,
                )
    ...
    def delete_events(
        ...
        for event in event_indices:
            if event < 0 or event >= len(self.event_locations):
                raise ValueError(f"Event {event} does not exist.")

        self.detected_events = [ev for idx, ev in enumerate(self.detected_events) if idx not in event_indices]
    ...
    # --- Event properties backed by individual Event objects ---
    @property
    def event_locations(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.location for e in self.detected_events], dtype=np.int64)
        return self._event_locations

    @event_locations.setter
    def event_locations(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.location = val
        else:
            self._event_locations = value

    @property
    def event_scores(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.score for e in self.detected_events], dtype=np.float64)
        return self._event_scores

    @event_scores.setter
    def event_scores(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.score = val
        else:
            self._event_scores = value

    @property
    def events(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.waveform for e in self.detected_events if e.waveform is not None])
        return self._events

    @events.setter
    def events(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.waveform = val
        else:
    ...
    @property
    def event_peak_locations(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.peak_location for e in self.detected_events], dtype=np.int64)
        return self._event_peak_locations

    @event_peak_locations.setter
    def event_peak_locations(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.peak_location = val
        else:
            self._event_peak_locations = value

    @property
    def bsl_starts(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.bsl_start for e in self.detected_events], dtype=np.int64)
        return self._bsl_starts

    @bsl_starts.setter
    def bsl_starts(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.bsl_start = val
        else:
            self._bsl_starts = value

    @property
    def bsl_ends(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.bsl_end for e in self.detected_events], dtype=np.int64)
        return self._bsl_ends

    @bsl_ends.setter
    def bsl_ends(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.bsl_end = val
        else:
            self._bsl_ends = value

    @property
    def event_start(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.onset_location for e in self.detected_events], dtype=np.int64)
        return self._event_start

    @event_start.setter
    def event_start(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.onset_location = val
        else:
            self._event_start = value

    @property
    def event_peak_values(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.peak_value for e in self.detected_events], dtype=np.float64)
        return self._event_peak_values

    @event_peak_values.setter
    def event_peak_values(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.peak_value = val
        else:
            self._event_peak_values = value

    @property
    def event_bsls(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.bsl_value for e in self.detected_events], dtype=np.float64)
        return self._event_bsls

    @event_bsls.setter
    def event_bsls(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.bsl_value = val
        else:
            self._event_bsls = value

    @property
    def event_bsl_durations(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.bsl_duration for e in self.detected_events], dtype=np.float64)
        return self._event_bsl_durations

    @event_bsl_durations.setter
    def event_bsl_durations(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.bsl_duration = val
        else:
            self._event_bsl_durations = value

    @property
    def decaytimes(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.decaytime for e in self.detected_events], dtype=np.float64)
        return self._decaytimes

    @decaytimes.setter
    def decaytimes(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.decaytime = val
        else:
            self._decaytimes = value

    @property
    def charges(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.charge for e in self.detected_events], dtype=np.float64)
        return self._charges

    @charges.setter
    def charges(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.charge = val
        else:
    ...
    @property
    def risetimes(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.risetime for e in self.detected_events], dtype=np.float64)
        return self._risetimes

    @risetimes.setter
    def risetimes(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.risetime = val
        else:
            self._risetimes = value

    @property
    def half_decay(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.half_decay for e in self.detected_events], dtype=np.float64)
        return self._half_decay

    @half_decay.setter
    def half_decay(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.half_decay = val
        else:
            self._half_decay = value

    @property
    def halfwidths(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.halfwidth for e in self.detected_events], dtype=np.float64)
        return self._halfwidths

    @halfwidths.setter
    def halfwidths(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.halfwidth = val
        else:
            self._halfwidths = value

    @property
    def rise_half_amp_times(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.rise_half_amp_time for e in self.detected_events], dtype=np.float64)
        return self._rise_half_amp_times

    @rise_half_amp_times.setter
    def rise_half_amp_times(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.rise_half_amp_time = val
        else:
            self._rise_half_amp_times = value

    @property
    def decay_half_amp_times(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.decay_half_amp_time for e in self.detected_events], dtype=np.float64)
        return self._decay_half_amp_times

    @decay_half_amp_times.setter
    def decay_half_amp_times(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.decay_half_amp_time = val
        else:
            self._decay_half_amp_times = value

    @property
    def min_positions_rise(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.min_position_rise for e in self.detected_events], dtype=np.float64)
        return self._min_positions_rise

    @min_positions_rise.setter
    def min_positions_rise(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.min_position_rise = val
        else:
            self._min_positions_rise = value

    @property
    def max_positions_rise(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.max_position_rise for e in self.detected_events], dtype=np.float64)
        return self._max_positions_rise

    @max_positions_rise.setter
    def max_positions_rise(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.max_position_rise = val
        else:
            self._max_positions_rise = value

    @property
    def min_values_rise(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.min_value_rise for e in self.detected_events], dtype=np.float64)
        return self._min_values_rise

    @min_values_rise.setter
    def min_values_rise(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.min_value_rise = val
        else:
            self._min_values_rise = value

    @property
    def max_values_rise(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.max_value_rise for e in self.detected_events], dtype=np.float64)
        return self._max_values_rise

    @max_values_rise.setter
    def max_values_rise(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.max_value_rise = val
        else:
            self._max_values_rise = value

    @property
    def slopes(self) -> np.ndarray:
        if self.detected_events:
            return np.array([e.slope for e in self.detected_events], dtype=np.float64)
        return self._slopes

    @slopes.setter
    def slopes(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.slope = val
        else:
    ...
    @property
    def exclude_events(self) -> np.ndarray:
        if self.detected_events:
            return np.array([1 if e.excluded else 0 for e in self.detected_events], dtype=np.int64)
        return np.zeros(len(self.event_locations), dtype=np.int64)

    @exclude_events.setter
    def exclude_events(self, value: np.ndarray) -> None:
        if self.detected_events:
            for ev, val in zip(self.detected_events, value):
                ev.excluded = bool(val)
    ...
    def eval_events(self, filter: bool = True) -> None:
        ...
        if self.event_locations.shape[0] > 0:
            ...
            for ix in range(self.event_locations.shape[0]):
                ev = Event(
                    location=self.event_locations[ix],
                    score=self.event_scores[ix] if hasattr(self, "event_scores") and len(self.event_scores) > ix else np.nan,
                    waveform=self.events[ix],
                    peak_location=self.event_peak_locations[ix],
                    peak_value=self.event_peak_values[ix],
                    bsl_start=self.bsl_starts[ix],
                    ...
                    min_value_rise=self.min_values_rise[ix],
                    max_value_rise=self.max_values_rise[ix],
                    slope=self.slopes[ix] if hasattr(self, "slopes") and len(self.slopes) > ix else np.nan,
                    excluded=False,
                )
                self.detected_events.append(ev)
            self.events = self.events - self.event_bsls[:, None]
