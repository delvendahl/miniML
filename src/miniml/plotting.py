...
from miniml.core.util import exp_fit
...
class Plotter:
    ...
    def plot_gradient_search(self, figsize: tuple[int, int] = (7, 4)):
        ...
        if self.detection.convolve_win > 0:
            filtered_trace = self.detection.hann_filter(
                data=self.detection.trace.data, filter_size=self.detection.convolve_win
            )
        else:
            filtered_trace = self.detection.lowpass_filter(
                data=self.detection.trace.data,
                cutoff=self.detection.trace.sampling_rate
                / self.detection.filter_factor,
                order=4,
            )

        filtered_prediction = maximum_filter1d(
            self.detection.prediction,
            size=int(5 * self.detection.interpol_factor),
            origin=-2,
        )

        axs[0].plot(filtered_prediction, self.main_trace_color)
        axs[0].scatter(
            self.detection.start_pnts,
            filtered_prediction[self.detection.start_pnts],
            c=self.red_color,
            zorder=2,
            label="start points",
        )
        axs[0].scatter(
            self.detection.end_pnts,
            filtered_prediction[self.detection.end_pnts],
            c=self.green_color,
            zorder=2,
            label="end points",
        )
        ...
        axs[1].scatter(
            self.detection.event_locations,
            filtered_trace[self.detection.event_locations],
            c=self.orange_color,
            zorder=2,
        ...
        axs[1].scatter(
            self.detection.start_pnts,
            filtered_trace[self.detection.start_pnts],
            c=self.red_color,
            zorder=2,
        )
        axs[1].scatter(
            self.detection.end_pnts,
            filtered_trace[self.detection.end_pnts],
            c=self.green_color,
            zorder=2,
        )
        ...
        axs[2].plot(
            self.detection.smth_gradient,
            self.main_trace_color,
            label="filtered gradient",
        )
        axs[2].axhline(
    ...
    def plot_event_overlay(
        ...
        event_x = np.arange(
            0,
            events.shape[1] * self.detection.trace.sampling,
        ...
        if not np.isnan(self.detection.avg_decay_fit_start):
            event_fit = (
                exp_fit(
                    event_x[self.detection.avg_decay_fit_start :],
                    *self.detection.avg_decay_fit,
                )
                * self.detection.event_direction
            )
            plt.plot(
                event_x[self.detection.avg_decay_fit_start :],
                event_fit,
    ...
    def plot_event_histogram(
        ...
        if plot == "frequency":
            data = np.diff(
                self.detection.event_locations * self.detection.trace.sampling,
                prepend=0,
            )
    ...
    def plot_prediction(
        ...
        if include_data:
            ax1 = plt.subplot(211)
        prediction_x = (
            np.arange(0, self.detection.prediction.shape[0])
            * self.detection.trace.sampling
        )
        ...
        if include_data:
            ...
            if plot_filtered_trace:
                if self.detection.convolve_win > 0:
                    main_trace = self.detection.hann_filter(
                        data=self.detection.trace.data,
                        filter_size=self.detection.convolve_win,
                    )
                else:
                    main_trace = self.detection.lowpass_filter(
                        data=self.detection.trace.data,
                        cutoff=self.detection.trace.sampling_rate
                        / self.detection.filter_factor,
                        order=4,
                ...
                plt.plot(
                    self.detection.trace.time_axis,
            ...
            plt.plot(
                self.detection.trace.time_axis, main_trace, c=self.main_trace_color
            )

            if hasattr(self.detection, "event_stats"):
                plt.scatter(
                    self.detection.event_peak_times,
                    main_trace[self.detection.event_peak_locations],
                    c=self.orange_color,
                    s=20,
                    zorder=2,
                ...
                if plot_event_params:
                    bsl_starts = (
                        self.detection.bsl_starts * self.detection.trace.sampling
                    )
                    bsl_ends = self.detection.bsl_ends * self.detection.trace.sampling

                    plt.scatter(
                        bsl_starts,
                        self.detection.event_bsls,
                        c=self.red_color,
                        zorder=2,
                        s=20,
                        label="baseline",
                    ...
                    half_decay_for_plot = self.detection.half_decay[
                        np.argwhere(~np.isnan(self.detection.half_decay)).flatten()
                    ].astype(np.int64)
                    half_decay_times_for_plot = self.detection.half_decay_times[
                        np.argwhere(
                            ~np.isnan(self.detection.half_decay_times)
                        ).flatten()
                    ]
                    plt.scatter(
                        half_decay_times_for_plot,
                        main_trace[half_decay_for_plot],
                        c=self.green_color,
                        s=20,
                        zorder=2,
                        label="half decay",
                    ...
                    plt.scatter(
                        self.detection.min_positions_rise,
                        self.detection.min_values_rise,
                        c=self.cyan_color,
                        s=20,
                        zorder=2,
                        label="10-90 rise",
                    ...
                    plt.scatter(
                        self.detection.max_positions_rise,
                        self.detection.max_values_rise,
                        c=self.cyan_color,
                        s=20,
                        zorder=2,
                    )
                ...
                plt.eventplot(
                    self.detection.event_peak_times,
                    lineoffsets=dat_min - data_range / 15,
                    linelengths=data_range / 20,
                    color="k",
                    lw=1.5,
                )
    ...
    def plot_event_locations(
        self, plot_filtered: bool = False, save_fig: str = ""
    ) -> None:
        """
        ...
        try:
            plt.scatter(
                self.detection.event_locations,
                self.detection.trace.data[self.detection.event_locations],
                c=self.orange_color,
                s=20,
                zorder=2,
            )
            data_range = np.abs(
                np.max(self.detection.trace.data) - np.min(self.detection.trace.data)
            )
    ...
    def plot_detection(self, save_fig: str = "") -> None:
        ...
        if hasattr(self.detection, "event_stats"):
            plt.scatter(
                self.detection.event_peak_times,
                self.detection.trace.data[self.detection.event_peak_locations],
                c=self.orange_color,
                s=20,
                zorder=2,
            )
            ...
            plt.eventplot(
                self.detection.event_peak_times,
                lineoffsets=dat_min - dat_range / 15,
                linelengths=dat_range / 20,
                color="k",
                lw=1.5,
            )
